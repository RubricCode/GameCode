...
def start(caller):
    ...
    """
    text = random_string_from_module(_CONNECTION_SCREEN_MODULE)
...
def ask_password(caller, string_input):
    ...
    banned = bans and (any(tup[0] == account.name.lower() for tup in bans) or any(
        tup[2].match(caller.address) for tup in bans if tup[2]))
...
def create_username(caller, string_input):
    ...
    if account:
        ...
        text = "Enter this account's new password. It must be at least 8 characters long"
        options = (
            {
                "key": "_default",
